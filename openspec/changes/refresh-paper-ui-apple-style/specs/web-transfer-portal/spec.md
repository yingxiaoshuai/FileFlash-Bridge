...
系统必须提供一个可由桌面和移动浏览器访问的响应式页面，展示当前设备名称、连接状态、文件上传区、**手机端已共享文件的下载区**、文本粘贴区和主要操作入口。页面必须在无需安装任何客户端的前提下完成向手机投递文件、从手机取回已共享文件、以及提交文本内容的核心流程。门户页的整体视觉风格必须与 App 主工作台保持一致，采用中性冷白或微灰的背景与表面基底、清晰的区块层级、统一的按钮和反馈样式，不得继续使用旧的浅黄色纸面风格。
